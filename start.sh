...
function usage
{
  ...
  ls -1 ${CONF_DIR}/env.* | awk -F"." '{print $2"."$3}' | sort -u | while read C
  do
...
function go
{
  ...
  if [[ ${CATENA} != "" ]]; then
    ...
    echo -e "########################################################################"
    echo -e "#"
    echo -e "#  - Per ogni Coda presente nei seguenti Moduli JMS:"
    echo -e "#"
    ...
    echo -e "#"
    echo -e "#  - Estraggo i seguenti valori per tutte le Destinazioni possibili:"
    echo -e "#"
    ...
    do
	  ...
	  sed -i "s/{{TITOLO}}/<h1>Report Code JMS<\/h1>/" ${MOD_SPOOL_DIR}/${MOD_FILENAME}.html
    ...
    ) | mailx -s "[${ENV} - ${CATENA}] Report Code JMS" ${DESTINATARI}
